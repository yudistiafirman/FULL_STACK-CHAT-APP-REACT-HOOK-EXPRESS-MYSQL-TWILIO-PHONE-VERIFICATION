    ...
    audioReciever:(req,res)=>{
        upload(req,res,(err)=>{
            try {
                ...
                //insert into db = result.insertId type audio
                db.query('insert into messages set?',[{senders_id:dataToken.id,recepient_id:recepientid,type:'audio'}],(err,result)=>{
                    try {
                        if(err)throw err
                        console.log(result)
                        db.query('insert into attachment set?',[{file_url:req.file.path,messages_id:result.insertId}],(err,response)=>{
                            try {
                                if(err)throw err
